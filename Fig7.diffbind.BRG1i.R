...
for (factor in factors) {
    ...
    filename <- paste0("project214.", factor, ".chipseq.rds")
	...
	filename <- paste0("project214.", factor,".", PBRM1_status, ".BRG1i.contrast.rds")
	saveRDS(contrast, filename)
	contrast <- readRDS(filename)
	
	for (diff_method in c("DBA_EDGER","DBA_DESEQ2")){
		# Plot MA plot
		pdf(paste0("project214.", factor, ".", PBRM1_status, ".BRG1i.MAplot.", 
			   diff_method,".pdf"))
		...
		if (exists("report.gain")){
			if (!is.null(report.gain)){
				...
				export.bed(report.gain,
					   paste0("project214.diffbind.786O.", factor, ".", 
						  PBRM1_status, ".BRG1i.gain.", 
						  diff_method,".bed")
					   )
			}
		}
		...
		if (exists("report.all")) {
			...
			export.bed(report.all,
				   paste0("project214.diffbind.786O.", factor, ".", 
					  PBRM1_status,".BRG1i.all.", diff_method,".bed")
				   )
		}
	}
    }
